...
97d275be6492  clams-nlp-example:0.0.5	 pipeline_tokenizer  0.0.0.0:5001->5000/tcp
...
When we run this script with `docker-compose up -d` three networked containers will be started. One container for the pipeline script and two containers for the two CLAMS applications, each of which is running a Flask or GUnicorn server on its standard port 5000, but exposing that port under a different port number so we can access it from the outside.
